...
async function playGeminiTTS(text) {
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      contents: [{
        parts: [{
          text: `다음 인도네시아어 문장을 원어민 발음으로 읽어주세요. 오디오만 반환하세요: "${text}"`
        }]
      }],
      generationConfig: {
...
async function playGoogleCloudTTS(text) {
  ...
  const modelName = localStorage.getItem('google_tts_model') || 'id-ID-Standard-C';
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      input: { text },
      voice: { languageCode: 'id-ID', name: modelName },
 ...
 */
function playWebSpeechTTS(text) {
  ...
  utterance.lang = 'id-ID'; // 인도네시아어 설정
  utterance.rate = 0.9;     // 학습을 위해 약간 천천히
  
  // 사용 가능한 목소리 중 인도네시아어 찾기
